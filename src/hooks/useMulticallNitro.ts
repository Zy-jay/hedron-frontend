...
import { ActiveHSIHEXStakeAuctionsList, LoanLiquidateStart } from "components/AuctionList/types";
import { LiquidationList } from "types/liquidationList";
...
export function useCustomCalls(): [number, EventData[]] {
  ...
  const [loanLiquidateStart, setLoanLiquidateStart] = useState<any[]>([]);

  async function getAuctions() {
    //  const loanLiquidate: LoanLiquidateStart =   eventsData[0].blockNumber
    // const provider = new ethers.providers.JsonRpcProvider("https://eth.llamarpc.com")
    // // const {provider} = useWeb3React()
  ...
  //   }); 
  return useMemo(() => {
    return [count, loanLiquidateStart]
  }, [count, loanLiquidateStart])
}
    // const calls = [
