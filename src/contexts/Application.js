...
import { HDRN_LOANS } from "apollo/queries"
dayjs.extend(utc)

const UPDATE = 'UPDATE'
const UPDATE_SESSION_START = 'UPDATE_SESSION_START'
const UPDATED_HDRN_LOANS = 'UPDATED_HDRN_LOANS'
const UPDATE_LATEST_BLOCK = 'UPDATE_LATEST_BLOCK'
const UPDATE_HEAD_BLOCK = 'UPDATE_HEAD_BLOCK'

const HDRN_LOANS_STATE = 'HDRN_LOANS'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATED_HDRN_LOANS: {
      const { hdrnLoans } = payload
      return {
        ...state,
        [HDRN_LOANS_STATE]: hdrnLoans,
      }
    }
  ...
  return (
    <ApplicationContext.Provider
      value={useMemo(
        ...
        [state, update, updateSessionStart, updateLatestBlock, updateHeadBlock]
      )}
    >
      {children}
    </ApplicationContext.Provider>
...
export function useListedTokens() {
  const [state, { updateHdrnLoans }] = useApplicationContext()
  const supportedTokens = state?.[UPDATED_HDRN_LOANS]
  ...
  }, [updateHdrnLoans, supportedTokens])

  return supportedTokens
}

export function useHdrnLoans() {
  const [state, { updateHdrnLoans }] = useApplicationContext()
  const hdrnLoans = state?.[HDRN_LOANS_STATE]
  ...
  useEffect(() => {
    async function fetch() {
     await client
        ...
        .then((res) => {
          const hdrnLoans = res.data.hdrnloans
          console.log(res)
          if (hdrnLoans) {
              updateHdrnLoans ()
          }
