...
export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "seminatempus/hedron-v2") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const HDRN_LOANS = (first?: number, skip?: number)=>{
  const queryString = `query ($first: Int, $skip: Int) {
    hdrnloans(
      first: ${first? first : 10 }
      skip: ${skip? skip : 0 }
      where: {isActive: true}
      orderBy: liquidationDay
      orderDirection: asc
    ) {
      hexStake {
        stakeId
...
// export const STATISTICS = gql`query () {
//     statistics(where: {id: "hdrnLoansActive"}) {
//       value
//       __typename
//     }
//   }
...
export const HDRN_LIQUIDATIONS = (first?: number, skip?: number)=>{
    const queryString = `query ($first: Int, $skip: Int) {
    hdrnliquidations(
        first: ${first? first : 10 }
        skip: ${skip? skip : 0 }
      where: {isActive: true}
      orderBy: liquidationStart
      orderDirection: asc
    ) {
      id
      loan {
...
export const HDRN_LIQUIDATIONS_ACTIVE = ()=>{
    const queryString = `query{
        statistics(where: {id: "hdrnLiquidationsActive"}) {
            value
            __typename
          }
  }
...
export const HDRN_LOANS_ACTIVE = ()=>{
    const queryString = `query{
        statistics(where: {id: "hdrnLoansActive"}) {
            value
            __typename
